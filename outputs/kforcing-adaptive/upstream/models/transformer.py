...
# Windows GPU teacher scoring reproduced a native exit crash with scripted
# helpers. Eager helpers preserve the arithmetic and avoid that execution path.
# An explicit override keeps the original path available for controlled probes.
HELPER_BACKEND = os.environ.get('KFORCING_HELPERS', 'eager' if os.name == 'nt' else 'script')
if HELPER_BACKEND not in ('eager', 'script'):
    raise ValueError('KFORCING_HELPERS must be eager or script')
if HELPER_BACKEND == 'script':
...
def _script_if_enabled(fn):
    return torch.jit.script(fn) if HELPER_BACKEND == 'script' else fn
...
def apply_rotary_pos_emb(
    ...
    if flash_attn is not None and qkv.is_cuda:
        return flash_attn.layers.rotary.apply_rotary_emb_qkv_(qkv, cos, sin)
    # Portable inference fallback. Match FlashAttention's non-interleaved RoPE;
    # rotate Q/K only, never V. Compute in fp32 then restore the input dtype.
    cos = torch.cat((cos, cos), dim=-1)[None, :, None, None, :]
    sin = torch.cat((sin, sin), dim=-1)[None, :, None, None, :]
    qk = qkv[:, :, :2].float()
    rotated = qk * cos.float() + rotate_half(qk) * sin.float()
    return torch.cat((rotated.to(qkv.dtype), qkv[:, :, 2:]), dim=2)
...
class DDiTBlock(nn.Module):
    ...
    def forward(
        ...
        if flash_attn is None:
            raise RuntimeError('FlashAttention is unavailable; use forward_inference for this portable pilot.')
